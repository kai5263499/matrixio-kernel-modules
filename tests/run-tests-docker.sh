...
# List of all available target configurations
ALL_TARGETS="pi4-legacy pi5-latest ci-standard"
...
case "$TEST_TARGET" in
    pi4-legacy|pi5-latest|ci-standard)
        # Single target
        config_result=$(get_kernel_config "$TEST_TARGET")
        if [[ $? -ne 0 ]]; then
            log "ERROR: Unknown test target: $TEST_TARGET"
            exit 1
        fi
        IFS=' ' read -r kernel_version arch <<< "$config_result"
    ...
    all)
        # All targets
        failed_configs=()
        for config in $ALL_TARGETS; do
            config_result=$(get_kernel_config "$config")
            IFS=' ' read -r kernel_version arch <<< "$config_result"
