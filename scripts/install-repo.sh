...
install_packages() {
    ...
    if [[ $REPLY =~ ^[Nn]$ ]]; then
        ...
        log "  APT: sudo apt install $PACKAGE_NAME"
        log "  Direct: curl -L -o package.deb https://kai5263499.github.io/matrixio-kernel-modules/pool/main/[package-file].deb && sudo dpkg -i package.deb"
        return
    fi
    ...
    if sudo apt install -y "$PACKAGE_NAME"; then
        ...
        warn "APT repository installation failed. Trying direct download..."
        log "Searching for direct download package..."
        
        # Try direct download as fallback
        DIRECT_URL="https://kai5263499.github.io/matrixio-kernel-modules/pool/main/"
        TEMP_DIR=$(mktemp -d)
        
        log "Downloading package list..."
        if curl -s "$DIRECT_URL" -o "$TEMP_DIR/index.html"; then
            # Extract .deb filename for our package from the HTML
            DEB_FILE=$(grep -o "${PACKAGE_NAME}[^\"]*\.deb" "$TEMP_DIR/index.html" | head -1)
            
            if [[ -n "$DEB_FILE" ]]; then
                log "Found package: $DEB_FILE"
                log "Downloading directly..."
                
                if curl -L "${DIRECT_URL}${DEB_FILE}" -o "$TEMP_DIR/$DEB_FILE"; then
                    log "Installing downloaded package..."
                    if sudo dpkg -i "$TEMP_DIR/$DEB_FILE"; then
                        log "Direct installation completed successfully!"
                    else
                        error "Direct installation failed."
                        log "You can try manual installation:"
                        log "  wget ${DIRECT_URL}${DEB_FILE}"
                        log "  sudo dpkg -i $DEB_FILE"
                    fi
                else
                    error "Failed to download package directly."
                fi
            else
                error "Could not find package file for $PACKAGE_NAME"
            fi
        else
            error "Could not access direct download repository."
        fi
